...
public class Hand {
	...
	private int Cards;
	private ArrayList<Hand> SaveExplodedHands;
	...
	static Hand EvaluateHand(Hand h) throws HandException {
		...
		h.SaveExplodedHands = ExplodedHands;
	...
	private static ArrayList<Hand> SubstituteCard(int iCardSub, ArrayList<Hand> hands) {
        ...
        for (Hand h : hands) {
            if ((h.getCardsInHand().get(iCardSub).isbWild() == true)
                ...
                for (Card JokerDeckCard : CreatedDeck.deckCards) {
	...
	private static Hand wildcardReplace(Hand h, Card c){
		...
		for(int i = 0; i < h.getCardsInHand().size(); i++){
			if (h.getCardsInHand().get(i) == c){
				h.getCardsInHand().remove(i);
				h.getCardsInHand().add(new Card(eSuit.CLUBS, eRank.JOKER, c.getiCardNbr()));
			}
			else{
			...
			if (h2.getHs().getKickers().size() > 1) {
				if (h1.getHs().getKickers().size() > 1) {
					result = h2.getHs().getKickers().get(eCardNo.SecondCard.getCardNo()).geteRank().getiRankNbr()
							- h1.getHs().getKickers().get(eCardNo.SecondCard.getCardNo()).geteRank()
									.getiRankNbr();
				}
				if (result != 0) {
					return result;
			...
			if (h2.getHs().getKickers().size() > 2) {
				if (h1.getHs().getKickers().size() > 2) {
					result = h2.getHs().getKickers().get(eCardNo.ThirdCard.getCardNo()).geteRank().getiRankNbr()
							- h1.getHs().getKickers().get(eCardNo.ThirdCard.getCardNo()).geteRank()
									.getiRankNbr();
				}
				if (result != 0) {
					return result;
			...
			if (h2.getHs().getKickers().size() > 3) {
				if (h1.getHs().getKickers().size() > 3) {
					result = h2.getHs().getKickers().get(eCardNo.FourthCard.getCardNo()).geteRank().getiRankNbr()
							- h1.getHs().getKickers().get(eCardNo.FourthCard.getCardNo()).geteRank()
									.getiRankNbr();
				}
				if (result != 0) {
					return result;
